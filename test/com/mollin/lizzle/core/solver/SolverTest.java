...
public class SolverTest {
    ...
    private Object[] parametersForEmptySolution() {
        GridInterface nonConstantGrid = Utils.getGridWithSomeActivatedCoords(2, 2, Arrays.asList(
                Coord.of(0, 0)
        ));
        return new Object[][]{
            // grille pleine -> pleine avec un pattern classique
            {Utils.getFullGrid(5, 5), Utils.getClassicPattern()},
            // grille pleine -> pleine avec un pattern vide
            {Utils.getFullGrid(2, 2), EMPTY_PATTERN},
            // grille vide -> vide avec un pattern classique
            {Utils.getEmptyGrid(5, 5), Utils.getClassicPattern()},
            // grille vide -> vide avec un pattern vide
            {Utils.getEmptyGrid(2, 2), EMPTY_PATTERN},
            // grille irrégulière -> irrégulière avec pattern classique
            {nonConstantGrid, Utils.getClassicPattern()},
    ...
    private Object[] parametersForNoSolution() {
        ...
        return new Object[][]{
            ...
            {Utils.getEmptyGrid(3, 3), Utils.getFullGrid(3, 3), EMPTY_PATTERN},
            // grille vide -> pleine avec un pattern 'impossible'
            {Utils.getEmptyGrid(5, 4), Utils.getFullGrid(5, 4), impossiblePattern},
            // grille vide -> grille inatteignable avec un pattern classique
            {Utils.getEmptyGrid(5, 5), Utils.getGridWithSomeActivatedCoords(5, 5, Coord.of(0, 0)), Utils.getClassicPattern()}
        };
    }
    ...
    private Object[] parametersForSpecificSolution() {
        return new Object[][]{
            {Utils.getEmptyGrid(5, 5), Utils.getClassicPattern(), new Solution(Coord.of(0, 0), Coord.of(3, 4))},
            {Utils.getEmptyGrid(3, 3), Utils.getClassicPattern(), new Solution(Coord.of(0, 0), Coord.of(1, 1), Coord.of(2, 2))}
        };
    }
    ...
    public void specificSolution(GridInterface startGrid, PatternInterface pattern, Solution solutionToFind) {
        ...
        GridInterface endGrid = Utils.getGridWithSomeActivatedCoords(startGrid.rows(), startGrid.columns(), activatedCoordsOfEndGrid);
        Solver solver = new Solver(startGrid, endGrid, pattern);
        Solutions solutions = solver.solve();
        Optional<Solution> firstSolution = solver.findFirstSolution();
        assertThat(solutions.getNbSolutions())
        ...
        for (int r = 0; r <= limitRow; r++) {
            for (int c = 0; c <= limitColumn; c++) {
                parameters.add(new Object[]{
                    Utils.getEmptyGrid(r, c),
                    Utils.getFullGrid(r, c),
                    Utils.getClassicPattern()
                });
            }
        }
